    ...
    for _,ent in pairs (pool) do
        ...
        if (water_pipe_input_entity) then
            if (water_pipe_input_entity.name ~= "offshore-pump") then
                ...
                ent.surface.create_entity({name="infinity-pipe", position= water_pipe_input_entity.position, force="player", direction=direction_for_offshore})
            end
            ...
            ent.surface.create_entity({name="infinity-pipe", position= {x_pos, y_pos}, force="player", direction=direction_for_offshore})
        end
    end
end
...
for key,ent in pairs(game.player.surface.find_entities_filtered({name={"pipe", "pipe-to-ground", "storage-tank", "pump", "infinity-pipe"}, force="player"})) do
    ent.destroy()
end
place_offshores(entity_pool)
